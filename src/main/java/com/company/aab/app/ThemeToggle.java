...
public class ThemeToggle extends Component implements ClickNotifier<ThemeToggle>,
    ...
    private Component galleryComponent;

    public ThemeToggle() {
        setIcon(getDefaultIcon());
        setGallery(new Image("dsds", "no"));
    }
    ...
    public void setGallery(@Nullable Component icon) {
        if (icon != null && icon.getElement().isTextNode()) {
            throw new IllegalArgumentException(
                    "Text node can't be used as an icon.");
        }
        if (galleryComponent != null) {
            remove(galleryComponent);
        }

        galleryComponent = icon;
        if (icon != null) {
            add(icon);
            updateGallerySlot();
        }
    ...
    private void updateGallerySlot() {
        iconComponent.getElement().setAttribute("slot", "gallery");
    }
